...
public class CmsCsvExportBean {
     ...
     * @throws SQLException propagated from {@link com.alkacon.opencms.formgenerator.database.CmsFormDataAccess#readFormFieldNames(String, long, long)}
     */
        ...
        if (m_formId != null) {
            try {
                List<CmsFormDataBean> forms = CmsFormDataAccess.getInstance().readForms(
                    ...
                    getEndTime().getTime());
                if (!forms.isEmpty()) {
                    CmsUUID formConfigStructureId = forms.get(0).getResourceId();
                    if (!formConfigStructureId.equals(CmsUUID.getNullUUID())) {
                        try {
                            CmsResource formConfigResource = m_cms.readResource(formConfigStructureId);
                            if (formConfigResource != null) {
                                setLocale(CmsLocaleManager.getMainLocale(m_cms, formConfigResource));
                                if (formConfigResource.getTypeId() == OpenCms.getResourceManager().getResourceType(
                                    CmsForm.TYPE_NAME).getTypeId()) {
                                    CmsJspContentAccessBean config = new CmsJspContentAccessBean(
                                        m_cms,
                                        m_locale,
                                        formConfigResource);
                                    if (config.getHasValue().get(CmsForm.NODE_OPTIONAL_CSV_EXPORT_CONFIGURATION).booleanValue()) {

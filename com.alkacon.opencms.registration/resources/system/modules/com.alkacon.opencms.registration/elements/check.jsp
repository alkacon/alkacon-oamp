<%@page buffer="none" session="false" import="org.opencms.i18n.*,org.opencms.jsp.*, com.alkacon.opencms.formgenerator.*, com.alkacon.opencms.registration.*, java.util.*" %><%
...
for (int i = 0, n = resultList.size(); i < n; i++) {
	...
	if (!CmsPasswordField.class.isAssignableFrom(current.getClass()) && !CmsHiddenField.class.isAssignableFrom(current.getClass()) && !CmsCaptchaField.class.isAssignableFrom(current.getClass())) {
...
if (captchaField != null) {
	...
	out.println("\t<img src=\"" + formHandler.link("/system/modules/com.alkacon.opencms.formgenerator/pages/captcha.jsp") + "?" + captchaSettings.toRequestParams(formHandler.getCmsObject()) + "\" width=\"" + captchaSettings.getImageWidth() + "\" height=\"" + captchaSettings.getImageHeight() + "\" alt=\"\" border=\"1\"><br>");
...
<table border="0" style="margin-top: 14px;">
<tr>
<form name="confirmvalues" method="post" enctype="multipart/form-data" action="<%= formHandler.link(formHandler.getRequestContext().getUri()) %>" onSubmit="return runConfirmValues();">
...
<form name="displayvalues" method="post" enctype="multipart/form-data" action="<%= formHandler.link(formHandler.getRequestContext().getUri()) %>">

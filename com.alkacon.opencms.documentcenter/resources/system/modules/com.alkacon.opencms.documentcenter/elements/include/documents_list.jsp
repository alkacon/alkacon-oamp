...
<%-- TITLE + DESCRIPTION --%>	
    <td><a href="<%= cms.link(currentDocument.getPath()) %>" <%= relAttrib %> title="<%= docTitle
		...
		if (!currentDocument.isFolder()) {     
			...
			while (i.hasNext()) {
				...
				if (locDoc != null) {
					...
					%>><a href="<%= cms.link(locDoc.getPath()) %>" <%= relAttrib %> title="<%= linkTitle %>"><%= loc %></a><%
					out.print("</td>");
				} 
...
<%-- FOLDER COLUMN --%>	
    <% if (showFolderColumn) {
          ...
          out.println( "<td><a href=\"" +cms.link(folderLink) +"\" title=\"" +messages.key("documentlist.link.open") +"\">" +folderName +"</a></td>" );
       } %>
<%-- TYPE //, 24, 16--%>    
  	<td>
		<a href="<%= cms.link(currentDocument.getPath()) %>" <%= relAttrib %> title="<%= messages.key("documentlist.link.open")%>">
		...
		if(currentDocument.hasTypes()){
			...
			while(it.hasNext()) {
			    ...
			    <a href="<%= cms.link(typeVersion.getPath()) %>" <%= relAttrib %> title="<%= messages.key("documentlist.link.open")%>"><%= cms.buildDocIcon(typeVersion.getPath(), messages, resources , currentDocument.isFolder()) %></a>
			    <%
			}
		}
		%>
...
<%-- DATE MODIFIED --%>
    <% if (showDateModified ) { %><td class="litte_font"><%
		if (!currentDocument.isFolder()) {
			String versionLink = uri;
			if (showFolderColumn) {
				versionLink = folderUri;
			}

...
<%
	...
	// get the main categories of the current folder
	CmsCategory categories = new CmsCategory(cms.getCmsObject(), folderUri, "category");
...
while (mainIterator.hasNext()) {
	...
	if (!"".equals(curMain.getTitle())) { 
	    %><h3><a href="<%= cms.link(curMain.getCmsResource()) %>"><%= curMain.getTitle() %></a></h3><%
	} else { 
		%>&nbsp;<%
	}
	...
	if( !subIterator.hasNext() ) {
		...
		while (subIterator.hasNext()) {
			...
			if (!"".equals(curSub.getTitle())) { 
				%><li class="nobullet"><a class="link-folder" href="<%= cms.link(curSub.getCmsResource()) %>"><%= curSub.getTitle() %></a></li><%
			}
		}
		out.println("</ul>");
	}

 ...
 * @since 6.0.0 
 */
public class CmsDocument extends Object {
    ...
    private static final Pattern PATTERN_ATT_NUMBER = Pattern.compile(".*(_)(\\d+)(\\.[^\\.]*)");
    ...
    private Map m_localizedDocuments;
    ...
    public CmsDocument(CmsResource resource, String path, Locale currentLocale, String title, String sortOrder) {
        ...
        setAttachments(new TreeMap());
        setLocalizedDocuments(new TreeMap());
    ...
    public List getAttachedDocuments() {
        ...
        return new ArrayList(getAttachments().values());
    }
     ...
     */
    public List getAttachments(Locale locale) {
    ...
    public List getLocales() {
        ...
        if (m_locales == null) {
            List result = new ArrayList(m_localizedDocuments.keySet().size() + 1);
            result.add(getLocale().toString());
            Iterator it = m_localizedDocuments.keySet().iterator();
            while (it.hasNext()) {
                result.add(it.next());
            }
            Collections.sort(result);
            m_locales = new ArrayList(result.size());
            for (int i = 0; i < result.size(); i++) {
                Locale loc = new Locale((String)result.get(i));
                m_locales.add(loc);
            }
        }
        return m_locales;
    ...
    public CmsDocument getLocalizedDocument(Locale locale, boolean returnDefault) {
        ...
        if (getLocale().equals(locale)) {
            ...
            doc = (CmsDocument)m_localizedDocuments.get(locale.toString());
            if ((doc == null) && returnDefault) {
                doc = (CmsDocument)m_localizedDocuments.get(getCurrentLocale().toString());
     ...
     */
    public Map getLocalizedDocuments() {
     ...
     */
    public void setLocalizedDocuments(Map localizedDocuments) {

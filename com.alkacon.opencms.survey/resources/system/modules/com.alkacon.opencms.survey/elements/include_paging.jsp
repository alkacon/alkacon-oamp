...
<div id="webformPaging">
	...
	<c:choose>
		<c:when test="${curPage <= 1}"><fmt:message key='report.prev_button.headline'/></c:when>
			<c:otherwise>
				<a href="<cms:link>${cms.requestContext.uri}?page=${curPage - 1}&amp;detail=true</cms:link>" title="<fmt:message key='report.prev_button.headline'/>"><fmt:message key="report.prev_button.headline"/></a>
			</c:otherwise>	
	</c:choose>
	...
	<c:forEach var="pageitem" begin="1" end="${fn:length(workBean.list)}">
		<c:if test="${ (pageitem >= (curPage-2)) && (pageitem <= (curPage+2)) }">
			<c:if test="${pageitem!=curPage}"><a href="<cms:link>${cms.requestContext.uri}?page=${pageitem}&amp;detail=true</cms:link>"><c:out value="${pageitem}"/></a></c:if>
	...
	<c:choose>
		...
		<c:otherwise>
			<a href="<cms:link>${cms.requestContext.uri}?page=${curPage + 1}&amp;detail=true</cms:link>" title="<fmt:message key='report.next_button.headline'/>"><fmt:message key="report.next_button.headline"/></a>
		</c:otherwise>	
	</c:choose>

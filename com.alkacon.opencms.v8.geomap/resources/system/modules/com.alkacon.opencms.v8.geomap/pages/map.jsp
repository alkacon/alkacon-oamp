<%@page import="com.alkacon.opencms.v8.geomap.*, org.opencms.file.*, org.opencms.main.*, org.opencms.util.*"%>
...
<fmt:bundle basename="com.alkacon.opencms.v8.geomap.frontend">
...
<c:choose>
	...
	<%
		...
		if (CmsStringUtil.isEmptyOrWhitespaceOnly(mapKey)) {
			mapKey = OpenCms.getModuleManager().getModule("com.alkacon.opencms.v8.geomap").getParameter("map.key", "");
		}
	...
	<c:forEach var="mapcoord" items="${map.valueList.MapCoord}" end="0">
		...
		function showGoogleMap() {
			...
			// create the map
			map = new google.maps.Map(document.getElementById("AlkaconV8GeoMapgooglemap"), mapOptions);
	...
	<c:choose>
			...
			<c:forEach var="mapcoord" items="${map.valueList.MapCoord}" varStatus="status">
				...
				%>
					...
					<c:choose>
						...
						<c:otherwise>
							contentString += "AlkaconV8GeoMapAddr";
		...
		function setMapInfoWindowContent(results, status, winIndex) {
			...
			if (status == google.maps.GeocoderStatus.OK) {
				if (results[0]) {
					infoContent = infoContent.replace(/AlkaconV8GeoMapAddr/, getMapInfoAddress(results[0]));
				}
			} else {
				if (status == google.maps.GeocoderStatus.OVER_QUERY_LIMIT && queryErrors <= 20) {
					...
					infoContent = infoContent.replace(/AlkaconV8GeoMapAddr/, "");
				}
			}
	...
	<div id="AlkaconV8GeoMapgooglemap" style="${mapw}${maph}${mapstyle}"></div>

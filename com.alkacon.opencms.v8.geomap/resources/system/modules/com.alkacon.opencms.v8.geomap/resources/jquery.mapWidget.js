(function($) {
    ...
    * @version 0.9
    */
   $.fn.AlkaconV8GeoMapMap = function(/**AsocArray*/options) /**jQuery*/ {
   
      // compute options before element iteration
      var /**AsocArray*/ config = $.extend({}, $.fn.AlkaconV8GeoMapMap.defaults, options ||
      {});
      ...
      return this.each(function() {
         ...
         $container.AlkaconV8GeoMapIdentify({
            prefix: baseId + '-',
            rules: [{
               find: 'label',
               attr: 'for'
         ...
         var /**LatLng*/ center = new google.maps.LatLng(vals.lat, vals.lng);
          ...
          *
          * @callee {null} not used
          * @param {LatLng} point where to center the map
          ...
          *
          * @callee {null} not used
          * @param {Placemark} place the selected address
          */
         var /**Function*/ selectAddress = function(/**Placemark*/place) {

            var /**LatLng*/ point = place.geometry.location;
         ...
         $address.bind('blur keydown', onReturnOrBlur(function() {
            ...
            geocoder.geocode({'address': address}, function(results, status) {
               ...
               // show the list
               $span.AlkaconV8GeoMapSuggest(data, opts);
               // style managing
         ...
         // style check boxes
         $allow.AlkaconV8GeoMapToggleBar();
         // style input text boxes
         $container.find('input[type=text]').AlkaconV8GeoMapInput();
         // style radio buttons
         $container.find('input[type=radio]').AlkaconV8GeoMapRadio();
         
         // enable option panel expander
         $container.find('a.collapsible').AlkaconV8GeoMapCollapsible({
    ...
    */
   $.fn.AlkaconV8GeoMapMap.defaults = {
      idPre: /**String*/ false,
   ...
   $(document).ready(function() {
      $('input[type=hidden].map-widget').each(function() {
         $(this).parent('div').AlkaconV8GeoMapMap({

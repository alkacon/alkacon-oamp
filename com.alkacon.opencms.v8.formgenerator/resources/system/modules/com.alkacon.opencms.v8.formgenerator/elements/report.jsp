 ...
 <div> 
	...
	<c:choose>
		<c:when test="${cms.edited}" >
			<div>
				<h3><%= report.getMessages().key("webformreport.init.pleaseReload") %></h3>
			</div>
		</c:when>
		<c:otherwise>

			<%-- Handle the case the page was recently reloaded, execute the scripts --%>											
<c:if test="${report.includeStyleSheet}"><cms:include file="/system/modules/com.alkacon.opencms.v8.formgenerator/elements/report_css.jsp" /></c:if>
<script type="text/javascript" src="<cms:link>/system/modules/com.alkacon.opencms.v8.formgenerator/resources/js/dateformat.js</cms:link>"></script>  
<script type="text/javascript" src="<cms:link><%= report.getVfsPathGridMessages() %></cms:link>"></script>  
<script type="text/javascript" src="<cms:link>/system/modules/com.alkacon.opencms.v8.formgenerator/resources/grid/grid/gt_grid_all.js</cms:link>"></script>
